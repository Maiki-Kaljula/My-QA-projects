...
describe('This is the first test suite, Maiki Kaljula', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        // Today's date is 17.02.2025
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Maiki')
        cy.get('[data-testid="lastNameTestId"]').type('Kaljula')
        cy.get('input[name="password"]').type('PassUpdated')
        cy.get('[name="confirm"]').type('PassUpdated')
    ...
    it('User can use only same both first and validation passwords', () => {
        ...
        cy.get('input[name="password"]').type('Parool479')
        cy.get('[name="confirm"]').type('Parool479123')
    ...
    it('User cannot submit data when username is absent', () => {
        ...
        cy.get('[data-testid="phoneNumberTestId"]').type('10203040')
        cy.get("input[name='password']").type('Parool479')
        cy.get('[name="confirm"]').type('Parool479')
